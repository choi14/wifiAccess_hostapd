...
static void update_channel_utilization(void *eloop_data, void *user_data)
{
	...
	//wpa_printf(MSG_INFO, "jychoi || bssLoad update_channel_utilization || sec: %u usec: %u", sec, usec); 
...
int bss_load_update_init(struct hostapd_data *hapd)
{
	...
	wpa_printf(MSG_INFO, "jychoi_update_init || update_period: %u timeout: %d sec: %u usec: %u",
		conf->bss_load_update_period, hapd->bss_load_update_timeout, sec, usec); 	
	return 0;
}
