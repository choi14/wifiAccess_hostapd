					      ...
					      struct survey_results *survey_res)
{
	...
	wpa_printf(MSG_INFO,
		   "jychoi || drvCallback single_channel_get_survey CU: %d freq: %d time: %ld busy: %ld",
		   iface->channel_utilization,
		   survey->freq,
		   (unsigned long int) survey->channel_time,
		   (unsigned long int) survey->channel_time_busy);
				     ...
				     struct survey_results *survey_results)
{
	...
	//wpa_printf(MSG_INFO, "jychoi || drvCallback hostapd_event_get_survey");
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_CONNECT_FAILED_REASON:
		...
		break;
	case EVENT_SURVEY:
		//wpa_printf(MSG_INFO, "jychoi || drvCallback wpa_supplicant_event");
